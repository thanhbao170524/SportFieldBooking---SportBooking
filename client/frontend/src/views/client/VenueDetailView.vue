  ...
  <div class="vdp">
    ...
    <div v-if="venueLoading" class="min-vh-100 placeholder-glow" style="pointer-events: none">
      ...
      <div class="container" style="margin-top: -30px; position: relative; z-index: 5">
        <div class="row g-4">
          <div class="col-lg-8">
            <div class="placeholder w-100 rounded-4"
              style="height: 600px; background: #ffffff; box-shadow: 0 4px 20px rgba(0,0,0,0.05)"></div>
          </div>
          <div class="col-lg-4">
            <div class="placeholder w-100 rounded-4"
              style="height: 500px; background: #ffffff; border: 1px solid #e2e8f0"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else>
      ...
      <div class="container vdp-main">
          ...
          <div class="col-lg-8">
            ...
            <div class="vdp-step border-bottom pb-4 mb-4">
              ...
              <div class="d-flex flex-wrap gap-3">
                <div v-for="c in venue.courts" :key="c.id"
                  ...
                  <div
                    :class="['p-2 rounded-2', isCourtSelected(c.id) ? 'bg-success text-white' : 'bg-success-subtle text-success']">
                  ...
                  <div class="flex-grow-1">
                    ...
                    <div class="d-flex align-items-center gap-2 mt-1">
                      <span v-if="c.sportType" class="badge bg-secondary bg-opacity-10 text-muted fw-bold"
                  ...
                  <div
                    :class="['rounded-circle border-2 border d-flex align-items-center justify-content-center flex-shrink-0', isCourtSelected(c.id) ? 'bg-success border-success text-white' : 'border-secondary bg-white']"
                    style="width:22px;height:22px;">
                    <svg v-if="isCourtSelected(c.id)" width="11" height="11" viewBox="0 0 24 24" fill="none"
            ...
            <div class="vdp-step border-bottom pb-4 mb-4">
              <div class="d-flex align-items-start gap-3 mb-3">
                ...
                <div>
                  <div class="fw-bold fs-6 text-dark">Chọn ngày</div>
                  <div class="text-muted small">Tất cả sân đã chọn sẽ đặt cùng ngày này</div>
                </div>
              </div>
              ...
              <div class="d-flex gap-2" style="overflow-x: auto; padding-bottom: 8px; scrollbar-width: none;">
                <div v-for="(d, i) in nextSevenDays" :key="d.full"
                  ...
                  style="min-width:70px;padding:10px 14px;cursor:pointer" @click="dateOffset = i; clearAllSlots()">
                  <span style="font-size:10px;font-weight:700;text-transform:uppercase;opacity:.7">{{ d.dayName
                    }}</span>
                ...
                <div
                  class="position-relative vdp-date-card d-flex flex-column align-items-center justify-content-center border-2 rounded-3 border bg-white flex-shrink-0"
                  ...
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" class="mb-1" stroke="#94a3b8"
                    stroke-width="2.3">
                  ...
                  <input type="date" ref="customDateInput" v-model="customDate" @change="dateOffset = 0"
                    class="position-absolute opacity-0"
                    style="width:1px;height:1px;bottom:0;right:0;pointer-events:none;" />
                </div>
              </div>
            </div>
            ...
            <div class="vdp-step border-bottom pb-4 mb-4">
              <div class="d-flex align-items-start gap-3 mb-3">
                ...
                <div class="flex-grow-1">
                  <div class="fw-bold fs-6 text-dark">Chọn khung giờ</div>
                  <div class="text-muted small">Chọn khung giờ cho từng sân đã chọn</div>
                </div>
                <!-- Nút chuyển đổi (Grid vs Timeline) được khôi phục -->
                <div class="btn-group btn-group-sm" role="group">
                  <button
                    :class="['btn px-3 fw-bold', slotView === 'grid' ? 'btn-success' : 'btn-outline-secondary bg-white']"
                    @click="slotView = 'grid'">
                  ...
                  <button
                    :class="['btn px-3 fw-bold', slotView === 'timeline' ? 'btn-success' : 'btn-outline-secondary bg-white']"
                    @click="slotView = 'timeline'">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <div v-if="selectedCourtIds.length === 0" class="text-center py-5 rounded-4 mt-2"
              ...
              <template v-else>
                ...
                <div class="d-flex gap-3 mb-3 flex-wrap">
                  <span class="d-flex align-items-center gap-1 text-muted small fw-semibold"><span
                      class="vdp-lgd rounded-2 shadow-sm" style="background:#fff;border:1.5px solid #d1d5db"></span>Còn
                    trống</span>
                  <span class="d-flex align-items-center gap-1 text-muted small fw-semibold"><span
                      class="vdp-lgd bg-success rounded-2 shadow-sm"></span>Đang chọn</span>
                  <span class="d-flex align-items-center gap-1 text-muted small fw-semibold"><span
                      class="vdp-lgd rounded-2 shadow-sm" style="background:#fca5a5;border:1px solid #f87171"></span>Đã
                    đặt</span>
                  <span class="d-flex align-items-center gap-1 text-muted small fw-semibold"><span
                      class="vdp-lgd rounded-2 shadow-sm"
                      style="background:#f1f5f9;border:1.5px solid #cbd5e1"></span>Đã khóa</span>
                  <span class="d-flex align-items-center gap-1 text-muted small fw-semibold"><span
                      class="vdp-lgd rounded-2 shadow-sm" style="background:#f8fafc;border:1px solid #cbd5e1"></span>Hết
                    hạn</span>
                </div>
                ...
                <div v-if="slotView === 'grid'" class="mt-3">
                  ...
                  <div class="d-flex gap-2 mb-3 flex-wrap">
                    <button v-for="cid in selectedCourtIds" :key="cid"
                      ...
                      {{ getCourtName(cid) }}
                      <!-- Badge lượng slot -->
                      <span v-if="(selectedSlotsByCourtId[cid] || []).length" class="badge rounded-pill ms-1"
                        :class="activeEditCourtId === cid ? 'bg-white text-success' : 'bg-success text-white'"
                    ...
                    <div v-for="slot in gridSlotsForActiveCourt" :key="slot.id" class="col-6 col-md-4 col-lg-3">
                      <div :class="['vdp-slot-card rounded-4 p-3 h-100 position-relative overflow-hidden',
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-1 relative z-2">
                          ...
                          <span v-else-if="slot.status === 'LOCKED'" class="badge bg-secondary text-white"
                            style="font-size:10px; background: #94a3b8 !important">Đã khóa</span>
                          <span v-else-if="isExpired(slot)" class="badge bg-secondary text-white"
                            style="font-size:10px; background: #94a3b8 !important">Hết hạn</span>
                          <span v-else-if="isSlotSelected(activeEditCourtId, slot)"
                            class="badge bg-white text-success shadow-sm" style="font-size:10px">Đã chọn</span>
                        </div>
                        <div class="fw-bold z-2 position-relative" style="font-size:12px; opacity:0.9">{{
                          formatPrice(slot.price) }} đ</div>
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    <div class="vdp-tl-rows">
                      <div v-for="court in selectedCourts" :key="court.id" class="vdp-tl-row d-flex">
                        ...
                        <div class="vdp-tl-slots position-relative d-flex">
                          <!-- Background Grid -->
                          <div v-for="h in tlHours" :key="h" :class="['vdp-tl-slot',
                          ...
                          <div v-for="seg in getTlSelectedSegs(court.id)" :key="`s${seg.start}`"
                            ...
                            :style="tlBlockStyle(seg.start, seg.end)" @click.stop="removeTlSegment(court.id, seg)">
                            <div class="vdp-tl-segment-content">
                              <span class="vdp-tl-segment-time">{{ tlFmtHour(seg.start) }} – {{ tlFmtHour(seg.end)
                                }}</span>
            ...
            <div class="vdp-step">
              <div class="d-flex align-items-start gap-3 mb-0" style="cursor:pointer" @click="svcOpen = !svcOpen">
                ...
                <div class="flex-grow-1">
                  <div class="fw-bold fs-6 text-dark d-flex align-items-center gap-2">Dịch vụ thêm<span
                      class="badge bg-secondary bg-opacity-10 text-muted fw-normal" style="font-size:11px">Tuỳ
                      chọn</span></div>
                  <div class="text-muted small">Nhấn để chọn dịch vụ đi kèm</div>
                </div>
                <svg :style="svcOpen ? 'transform:rotate(180deg)' : ''" style="transition:.25s;flex-shrink:0;color:#94a3b8"
              ...
              <div v-if="!svcOpen && selectedServices.length" class="d-flex flex-wrap gap-2 mt-2">
                <span v-for="sid in selectedServices" :key="sid"
                  class="badge rounded-pill d-flex align-items-center gap-1"
                  style="background:#dcfce7;color:#15803d;border:1px solid #bbf7d0;font-size:12px;font-weight:600;padding:4px 12px">
                  {{ getServiceName(sid) }}
                  <span v-if="getServicePrice(sid) > 0" class="ms-1" style="opacity:0.8">(+{{
                    formatPrice(getServicePrice(sid))
                    }})</span>
                  <button @click.stop="toggleService(sid)" class="btn-close btn-close-sm ms-1"
                ...
                <div v-if="svcOpen" class="mt-3">
                  <div class="row g-2">
                    <div v-for="svc in services" :key="svc.id" class="col-12 col-md-6">
                      <div
                        :class="['vdp-svc-card d-flex align-items-center gap-3 p-3 rounded-3 border-2 border', isServiceSelected(svc.id) ? 'border-success bg-success-subtle' : 'border-light-subtle bg-white']"
                        style="cursor:pointer" @click="toggleService(svc.id)">
                        <div
                          :class="['p-2 rounded-2 d-flex align-items-center justify-content-center', isServiceSelected(svc.id) ? 'bg-success text-white' : 'bg-success-subtle text-success']">
                          <span class="material-icons" style="font-size: 20px;">{{ svc.icon }}</span>
                        </div>
                        <div class="flex-grow-1">
                          <div class="fw-bold small text-dark">{{ svc.name }}</div>
                          <div v-if="svc.price > 0" class="text-success fw-bold" style="font-size:12px">+{{
                            formatPrice(svc.price)
                            }} đ</div>
                          <div v-else class="text-muted" style="font-size:11px; font-style: italic;">Miễn phí</div>
                        </div>
                        <div
                          :class="['rounded-circle border-2 border d-flex align-items-center justify-content-center', isServiceSelected(svc.id) ? 'bg-success border-success text-white' : 'border-secondary']"
                          style="width:20px;height:20px;flex-shrink:0">
                          <svg v-if="isServiceSelected(svc.id)" width="10" height="10" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="3">
                            <polyline points="20 6 9 17 4 12" />
                          </svg>
                        </div>
                  ...
                  <div class="text-end mt-3">
                    <button class="btn btn-success btn-sm" @click="svcOpen = false">✓ Xong — {{ selectedServices.length }}
                      dịch vụ đã
                      chọn</button>
                  </div>
                </div>
          ...
          <div class="col-lg-4">
            <div class="vdp-sidebar card border-0 sticky-top" style="top:90px; background: #ffffff;">
              <div class="card-header d-flex justify-content-between align-items-center py-3 px-4">
                ...
                <span v-if="totalSelectedSlotsCount" class="badge rounded-pill"
                  style="background: rgba(255,255,255,0.2); backdrop-filter: blur(4px); font-size: 13px; font-weight: 700;">{{
                  selectedCourtIds.length }} sân</span>
              </div>

              <!-- Empty state -->
              <div v-if="!totalSelectedSlotsCount" class="text-center py-5 text-muted">
                <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" stroke-width="1.3"
              ...
              <div v-else class="card-body p-0">
                <!-- Ngày chọn -->
                <div class="mx-4 mt-4 mb-3 p-3 d-flex align-items-center gap-3"
                  style="background:#f8fafc; border:1.5px solid #e2e8f0; border-radius:12px; box-shadow: inset 0 2px 4px rgba(0,0,0,0.01)">
                  <div
                    style="width:42px; height:42px; border-radius:10px; background:#198754; color:#fff; display:flex; align-items:center; justify-content:center; flex-shrink:0; box-shadow: 0 2px 6px rgba(22,163,74,0.15)">
                  ...
                  <div v-for="cid in selectedCourtIds" :key="cid" class="mb-3">
                    <template v-if="(selectedSlotsByCourtId[cid] || []).length">
                      ...
                      <div class="d-flex align-items-center gap-2 mb-2">
                        <div class="bg-success text-white px-3 py-1 d-flex align-items-center gap-1 shadow-sm"
                          style="font-size:12px;font-weight:700; border-radius: 10px">
                      ...
                      <div class="border rounded-4 p-2 bg-white shadow-sm"
                        style="border: 1.5px solid #e2e8f0 !important; border-radius: 12px !important">
                        <div v-for="g in mergedSlotsByCourt(cid)" :key="g.timeLabel"
                          class="d-flex align-items-center gap-2 p-2 rounded-3 mb-1 vdp-slot-row">
                          ...
                          <button class="vdp-rm-btn shadow-sm"
                            @click="g.slots.forEach(s => toggleSlot(cid, s))">×</button>
                        </div>
                        <!-- Subtotal sân này -->
                        <div class="d-flex justify-content-between px-2 pt-2 border-top mt-1">
                          <span class="text-muted fw-bold" style="font-size:12px; letter-spacing: 0.5px">Cộng {{
                            getCourtName(cid) }}</span>
                          <span class="fw-black text-dark" style="font-size:14px">{{ formatPrice(courtTotalByCourt(cid))
                            }} đ</span>
                        </div>
                      </div>
                    </template>
                ...
                <div v-if="selectedServices.length" class="px-4 mt-3">
                  <div class="d-flex align-items-center gap-2 mb-2">
                    ...
                    <div class="text-uppercase fw-black" style="font-size:12px;letter-spacing:1px;color:#0f172a">Dịch vụ
                      đi kèm
                    </div>
                  </div>
                  <div class="border rounded-4 p-2 bg-white shadow-sm"
                    style="border: 1.5px solid #e2e8f0 !important; border-radius: 12px !important">
                    <div v-for="sid in selectedServices" :key="sid"
                      class="d-flex align-items-center gap-2 p-2 rounded-3 mb-1 vdp-slot-row">
                      <span class="flex-grow-1 small fw-bold text-dark">{{ getServiceName(sid) }}</span>
                      <span v-if="getServicePrice(sid) > 0" class="fw-bold text-success small">{{
                        formatPrice(getServicePrice(sid)) }} đ</span>
                ...
                <div class="px-4 mt-4">
                  <div class="input-group"
                    style="border-radius:12px; border: 1.5px solid #e2e8f0; overflow:hidden; box-shadow:0 2px 6px rgba(0,0,0,0.04)">
                    ...
                    <input v-model="form.voucher" type="text" class="form-control border-0 shadow-none ps-2 fw-bold"
                      :class="voucherApplied ? 'is-valid' : voucherError ? 'is-invalid' : ''" placeholder="Mã voucher giảm giá"
                      style="font-size: 14px; color: #1e293b" />
                    <button class="btn btn-success fw-bold px-3 transition border-0" :disabled="!form.voucher"
                      @click="applyVoucher"
                      style="font-size:13px; letter-spacing: 0.5px; border-radius: 0">DÙNG</button>
                  </div>
                ...
                <div class="px-4 mt-4 mb-2">
                  <div class="d-flex align-items-center gap-2 mb-3">
                    ...
                    <div class="text-uppercase fw-black" style="font-size:13px;letter-spacing:1px;color:#0f172a">Thông
                      tin người
                      đặt</div>
                  </div>
                  <div class="mb-3">
                    <label class="form-label small fw-bold mb-1 text-dark">Họ và tên <span
                        class="text-danger">*</span></label>
                    <input v-model="form.fullname" type="text" class="form-control py-2 shadow-sm"
                      style="border-radius:12px; font-weight:600; border: 1.5px solid #e2e8f0" placeholder="..." />
                  </div>
                  <div class="mb-3">
                    <label class="form-label small fw-bold mb-1 text-dark">Số điện thoại <span
                        class="text-danger">*</span></label>
                    <input v-model="form.phone" type="tel" class="form-control py-2 shadow-sm"
                      style="border-radius:12px; font-weight:600; border: 1.5px solid #e2e8f0" placeholder="..." />
                  </div>
                  <div class="mb-3">
                    <label class="form-label small fw-bold mb-1 text-dark">Email <span
                        class="text-muted fw-normal">(Không bắt
                        buộc)</span></label>
                    <input v-model="form.email" type="email" class="form-control py-2 shadow-sm"
                      style="border-radius:12px; font-weight:600; border: 1.5px solid #e2e8f0" placeholder="..." />
                  </div>
                  <div class="mb-0">
                    <label class="form-label small fw-bold mb-1 text-dark">Ghi chú yêu cầu <span
                        class="text-muted fw-normal">(Tuỳ
                        chọn)</span></label>
                    <textarea v-model="form.note" class="form-control py-2 shadow-sm"
                      style="border-radius:12px; font-weight:600; border: 1.5px solid #e2e8f0" rows="2"
                ...
                <!-- Tổng cộng -->
                <div class="p-4 mt-4"
                  style="background: linear-gradient(135deg, #f8fafc, #f1f5f9); border-radius: 0 0 16px 16px; border-top: 1px solid #e2e8f0;">
                  <div v-if="serviceTotal > 0" class="d-flex justify-content-between fw-bold text-muted py-1"
                    style="font-size:14px"><span>Tiền sân ({{ selectedCourtIds.length }} sân)</span><span>{{
                      formatPrice(courtTotal) }} đ</span></div>
                  <div v-if="serviceTotal > 0" class="d-flex justify-content-between fw-bold text-muted py-1"
                    style="font-size:14px"><span>Dịch vụ thêm</span><span>{{ formatPrice(serviceTotal) }} đ</span></div>
                  <div v-if="discount > 0" class="d-flex justify-content-between fw-bold py-1" style="font-size:14px">
                    <span class="text-muted">Giảm giá voucher</span><span class="text-danger fw-black">– {{
                  ...
                  <div class="d-flex justify-content-between align-items-end pt-3 mt-2 border-top border-2">
                    ...
                    <span class="fw-black text-success" style="font-size:26px; line-height: 1">{{
                      formatPrice(grandTotal) }}
                      đ</span>
                  </div>

                  <button
                    class="btn btn-success w-100 fw-black shadow px-4 py-3 mt-4 d-flex align-items-center justify-content-center gap-2"
                    style="font-size:16px; border-radius:12px; letter-spacing:1px"
                    :disabled="!form.fullname || !form.phone" @click="handleBooking">
                    XÁC NHẬN ĐẶT SÂN
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                      <polyline points="9 18 15 12 9 6"></polyline>
                    </svg>
                  </button>
                  <div v-if="!form.fullname || !form.phone" class="text-danger text-center mt-3 fw-bold"
                    style="font-size:13px;">Vui
                    lòng điền họ tên và số điện thoại!</div>
                  <div class="text-muted text-center mt-2 fw-semibold" style="font-size:12px; opacity:0.8;">Bằng việc
                    xác nhận,
                    bạn đồng ý với chính sách hoàn hủy.</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <!-- Xem chi tiết sân -->
        <VenueInfoTab v-if="activeTab === 'info'" :venue="venue" :venue-images="venueImages"
          @switch-to-booking="setActiveTab('booking')" />

        <!-- Tab đánh giá -->
        <VenueReviewTab v-if="activeTab === 'review'" :club-id="clubId" @switch-to-booking="setActiveTab('booking')"
          @open-lightbox="({ images, index }) => openLightbox(images, index)" />

      </div>

      <!-- A. STICKY BOTTOM BAR FOR MOBILE -->
      <div v-if="activeTab === 'booking' && !venueLoading"
        class="vdp-mobile-bar d-lg-none position-fixed bottom-0 start-0 w-100 bg-white shadow-lg p-3 d-flex justify-content-between align-items-center"
        style="z-index: 1050; border-top: 1px solid #e2e8f0; border-radius: 20px 20px 0 0; padding-bottom: calc(env(safe-area-inset-bottom) + 16px) !important;">
        <div>
          <div class="text-muted fw-bold text-uppercase" style="font-size: 11px; letter-spacing: 0.5px">Tổng cộng giỏ
          </div>
          <div class="fw-black text-success fs-4 lh-1 mt-1">{{ formatPrice(grandTotal) }} đ</div>
        </div>
        <button class="btn btn-success fw-bold px-4 py-2 rounded-pill shadow-sm d-flex align-items-center gap-2"
          :disabled="!form.fullname || !form.phone || totalSelectedSlotsCount === 0" @click="handleBooking"
    ...
    <transition name="fade">
      <div v-if="lightbox.show" class="vdp-lightbox" @click.self="lightbox.show = false">
        <button class="vdp-lightbox__close" @click="lightbox.show = false"><svg width="20" height="20" viewBox="0 0 24 24"
            fill="none" stroke="currentColor" stroke-width="2.5">
            <line x1="18" y1="6" x2="6" y2="18" />
            <line x1="6" y1="6" x2="18" y2="18" />
          </svg></button>
        <button class="vdp-lightbox__nav vdp-lightbox__nav--prev"
          @click="lightbox.index = (lightbox.index - 1 + lightbox.images.length) % lightbox.images.length"><svg width="20"
            height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
            <polyline points="15 18 9 12 15 6" />
          </svg></button>
        <transition name="gfade" mode="out-in"><img :key="lightbox.index" :src="lightbox.images[lightbox.index]"
            class="vdp-lightbox__img" /></transition>
        <button class="vdp-lightbox__nav vdp-lightbox__nav--next"
          @click="lightbox.index = (lightbox.index + 1) % lightbox.images.length"><svg width="20" height="20"
