<template>
  <div class="row g-4 bg-white shadow-sm p-4 vdp-panel-container">
    <div class="col-lg-8">
      ...
      <div v-if="venueImages.length" class="vdp-gallery mt-4 mb-2">
        ...
        <div class="vdp-gallery__main position-relative overflow-hidden mb-3" style="height:380px;background:#0f172a;">
          <transition name="gfade" mode="out-in">
            <img :key="galleryIndex" :src="venueImages[galleryIndex]?.url" :alt="venueImages[galleryIndex]?.caption"
              class="position-absolute w-100 h-100" style="object-fit:cover;top:0;left:0;" />
          </transition>
          <div class="position-absolute bottom-0 start-0 end-0"
            style="height:110px;background:linear-gradient(to top,rgba(0,0,0,.72) 0%,transparent 100%);pointer-events:none;z-index:2">
          </div>
          <div class="position-absolute bottom-0 start-0 px-4 pb-3" style="z-index:3">
            <span class="text-white fw-semibold d-flex align-items-center gap-2"
              style="font-size:14px;text-shadow:0 1px 6px rgba(0,0,0,.6)">
          ...
          <div class="position-absolute bottom-0 start-0 end-0 d-flex justify-content-center gap-1 pb-3"
            style="z-index:3;pointer-events:none">
            <span v-for="(_, i) in venueImages" :key="i" class="vdp-gal-dot"
              :class="{ 'vdp-gal-dot--active': i === galleryIndex }"></span>
          </div>
          <button class="vdp-gal-nav vdp-gal-nav--prev" @click="prevImage"><svg width="18" height="18"
              viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
              <polyline points="15 18 9 12 15 6" />
            </svg></button>
          <button class="vdp-gal-nav vdp-gal-nav--next" @click="nextImage"><svg width="18" height="18"
              viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
              <polyline points="9 18 15 12 9 6" />
            </svg></button>
        </div>
        <div class="d-flex gap-2 pb-1"
          style="overflow-x:auto;scrollbar-width:thin;scrollbar-color:#e2e8f0 transparent;">
          <div v-for="(img, i) in venueImages" :key="i" class="vdp-gal-thumb flex-shrink-0 overflow-hidden"
            :class="{ 'vdp-gal-thumb--active': i === galleryIndex }" @click="galleryIndex = i">
      ...
      <div v-if="courtsWithImages.length" class="vdp-court-photos mt-4 mb-2">
        ...
        <div v-for="court in courtsWithImages" :key="court.id"
          class="vdp-court-photo-block mb-4 pb-4 border-bottom border-light-subtle">
          <div class="d-flex align-items-center justify-content-between gap-3 mb-2 flex-wrap">
            <div class="d-flex align-items-center gap-2 flex-wrap">
              ...
              <span v-if="court.sportType" class="badge bg-secondary bg-opacity-10 text-muted fw-bold"
                style="font-size:10px">{{ formatSport(court.sportType) }}</span>
              <span class="text-muted small fw-semibold" v-if="court.images?.length">({{ court.images.length }}
                ảnh)</span>
            </div>
            <button v-if="(court.images?.length || 0) > 3" type="button" class="btn btn-sm btn-outline-success fw-bold"
              style="border-radius: 10px;" @click="openCourtGallery(court)">
              Xem tất cả
            </button>
          </div>
          <div class="row g-2">
            <div v-for="(img, idx) in (court.images || []).slice(0, 3)" :key="court.id + '-' + idx"
              class="col-6 col-md-4">
              <div class="ratio ratio-4x3 rounded-3 overflow-hidden border shadow-sm bg-light">
                <img :src="img.url" :alt="img.caption || court.name" class="w-100 h-100" style="object-fit:cover"
                  loading="lazy" />
              </div>
      ...
      <div v-for="(d, i) in venue.openingHours" :key="d.day"
        :class="['d-flex justify-content-between px-3 py-2 rounded-2', i % 2 === 1 ? 'bg-light' : '']">
        <span class="fw-semibold small">{{ d.day }}</span>
        <span class="fw-bold small" :class="d.isClosed ? 'text-danger' : 'text-success'">{{ d.isClosed ? 'Đóng cửa' : `${d.open}
          – ${d.close}` }}</span>
      </div>
      ...
      <div class="row g-2">
        <div v-for="a in venue.amenities" :key="a.id" class="col-6 d-flex align-items-center gap-2">
          <div
            class="bg-success-subtle text-success rounded-circle d-flex align-items-center justify-content-center shadow-sm"
          ...
          <span v-if="a.price > 0"
            ...
            style="font-size:10px">{{ formatPrice(a.price) }}</span>
      ...
      <div class="mb-3 d-flex align-items-center gap-2 text-muted small fw-semibold" style="line-height: 1.5">
        <svg style="flex-shrink:0" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
          stroke-width="2.5">
      ...
      <div class="overflow-hidden border shadow-sm" style="border-radius:12px; height: 320px;">
        <iframe v-if="venue.address" width="100%" height="100%" frameborder="0" style="border:0"
    ...
    <div class="col-lg-4">
      <div class="vdp-sidebar card border-0">
        ...
        <div class="card-body"><button class="btn btn-success w-100 fw-bold" @click="$emit('switch-to-booking')">Chọn
            khung giờ →</button></div>
      </div>
    </div>
  </div>
  ...
  <transition name="fade">
    <div v-if="courtGallery.show" class="vdp-court-modal" role="dialog" aria-modal="true"
      :aria-label="`Ảnh sân ${courtGallery.courtName}`" @click.self="closeCourtGallery">
      <div class="vdp-court-modal__content">
        <div class="d-flex align-items-center justify-content-between gap-2 mb-3">
          <div class="fw-black">{{ courtGallery.courtName }}</div>
          <button type="button" class="btn btn-sm btn-light border" @click="closeCourtGallery"
            aria-label="Đóng">×</button>
        </div>

        <div class="vdp-court-modal__viewer position-relative overflow-hidden rounded-4 border bg-dark">
          <transition name="gfade" mode="out-in">
            <img :key="courtGallery.index" :src="courtGallery.images[courtGallery.index]?.url"
              :alt="courtGallery.images[courtGallery.index]?.caption || courtGallery.courtName" class="w-100 h-100"
              style="object-fit:contain; max-height: 70vh;" />
          </transition>

          <button class="vdp-court-modal__nav vdp-court-modal__nav--prev" type="button" @click="prevCourtImage"
            aria-label="Ảnh trước">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.8">
              <polyline points="15 18 9 12 15 6" />
            </svg>
          </button>
          <button class="vdp-court-modal__nav vdp-court-modal__nav--next" type="button" @click="nextCourtImage"
        ...
        <div v-if="courtGallery.images.length > 1" class="d-flex gap-2 mt-3 pb-1"
          style="overflow-x:auto;scrollbar-width:thin;scrollbar-color:#e2e8f0 transparent;">
          <button v-for="(img, i) in courtGallery.images" :key="`thumb-${i}`" type="button"
            ...
            @click="courtGallery.index = i" :aria-label="`Xem ảnh ${i + 1}`">
            <img :src="img.url" :alt="img.caption || courtGallery.courtName"
              style="width:92px;height:64px;object-fit:cover;border-radius:10px;border:1px solid #e2e8f0;display:block" />
          </button>
        </div>
      </div>
    </div>
...
.vdp-court-modal__nav:hover {
  background: rgba(0, 0, 0, 0.55);
}

.vdp-court-modal__nav--prev {
  left: 12px;
}

.vdp-court-modal__nav--next {
  right: 12px;
}
